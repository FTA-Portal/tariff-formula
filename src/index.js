...
var dumpHelper = function(tree)
{
  ...
  if (tree.friendly)
    suffix = ' friendly \'' + tree.friendly + '\'' + suffix;

  if (tree.units)
    suffix = " u '" + tree.units + "'" + suffix;

  if (tree.quantity)
    return tree.quantity + suffix;
  ...
  if (tree.variable)
    return 'var_' + tree.variable + suffix;

  if (tree.error)
    return 'error' + suffix;

  if (tree.group)
    return "(" + tree.group.map(dumpHelper).join('') + ')' + suffix;

  console.error(tree);
};

var signatureHelper = function(tree)
{
  var suffix = '';
  ...
  if (tree.group)
    return "(" + tree.group.map(signatureHelper).join('') + ')' + suffix;

  console.error('ERRIR SUGBATYRE', tree);
};

var unitHelper = function(tree)
{
  if (tree.units)
  {
    return tree.units;
...
var quantityHelper = function(tree)
{
  if (tree.quantity)
  {
    try
...
var numberFormatHelper = function(number)
{
  try
  {
...
var friendlyHelper = function(tree, level)
{
  var prefix = '';
  var suffix = '';
  level = level || 0;
  ...
  if (tree.variable)
    return '' + tree.variable.replace(/_/g, ' ') + '';

  if (tree.error)
    return ('error' + suffix).replace(/^\s+|\s+$/g, '');

  if (tree.group)
    return ("(" + tree.group.map(friendlyHelper).join('') + ')' + suffix).replace(/^\s+|\s+$/g, '');

  console.error(error);
};

var variablesHelper = function(tree, vars)
{
  const
  allvars = vars || {};
...
var evaluateHelper = function(tree, variables)
{
  var result = {
    units : tree.units
  };
